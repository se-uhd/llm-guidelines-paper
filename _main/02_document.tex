...
We do not address multi-modal foundation models (FM) that support other media such as images.
...
Although full reproducibility of LLM study results is very challenging given LLM's non-determinism, transparency on LLM usage, methods, data, and architecture, as suggested by our guidelines, is an essential prerequisite for future replication studies. \added{Finally, for \emph{peer review of LLM papers}, we suggest reviewers can simply check the provided guidelines as a start. However, there are also things reviewers should avoid, such as excessive requirements regarding verification. Future work for these guidelines should include \emph{invalid critiques} as well, as is done in the SIGSOFT Empirical Standards work~\cite{ralph2021empiricalstandardssoftwareengineering}.}
