...
In any event, as soon as an LLM is involved, the information outlined in this guideline is essential for reproducibility.

\todo{Mention that model quantization should also be reported (for open models, regardless if self-hosted or cloud-based) \url{https://developer.nvidia.com/blog/model-quantization-concepts-methods-and-why-it-matters/}}

\guidelinesubsubsection{Recommendations}

LLMs or LLM-based tools, especially those offered as services, are frequently updated; different versions may produce different results for the same input.
...
\item Checksum/fingerprint of used model version and configuration (\added{\should});
\item Additional properties such as context window size (\added{\should}).
\end{enumerate*}
...
Setting the temperature to 0 configures greedy decoding, i.e., always selecting the most probable next token, which minimizes output variability across runs.
However, this involves a trade-off: greedy decoding can degrade output quality by producing repetitive text and missing higher-quality responses that lie outside the single most probable token path.

Moreover, even with a temperature of 0, full determinism is rarely guaranteed.
Floating-point arithmetic on GPUs is sensitive to hardware and parallelization strategies, causing slight numerical differences that cascade into divergent token selections~\cite{YuanNondeterminismLLMInference}, and architectural choices such as Sparse Mixture-of-Experts routing amplify this effect~\cite{Chann2023}.
For commercial APIs, silent backend changes can produce different outputs over time~\cite{DBLP:journals/corr/abs-2307-09009}, and even self-hosted open models with identical settings do not always yield consistent outputs~\cite{DBLP:journals/corr/abs-2510-25506, DBLP:conf/naacl/SongWLL25}.
\added{Researchers should therefore not treat a temperature of 0 as a guarantee of reproducibility, but as one measure among several---including fixed seed values~\cite{OpenAI23}, system fingerprints, and archiving of raw outputs.
When chosen primarily for reproducibility, this motivation \should be stated explicitly, along with an acknowledgment of its potential impact on output quality.}
...
\added{In most cases, reviewer should simply make a list of missing information about versions, configurations, parameters, etc. and request that the missing information be added. Such requests are typical of minor revisions unless the reviewer suspects that missing information obscures deeper methodological problems. Before rejecting a paper over missing information reviewers \textit{must} thoroughly read the paper and check appendices and supplementary materials. \textit{Many} papers are rejected for ``missing'' information that has been clearly disclosed in an appendix or online supplement and the structure of our review processes makes such mistakes easy to rectify in a revision but impossible in a rejection. Reviewers should therefore exercise caution in rejecting papers over missing details. If the paper provides source code to voluminous for the reviewer to inspect, the reviewer should ask ``if this code compiles, must it include the seemingly missing details?''} 