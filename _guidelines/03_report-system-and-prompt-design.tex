...
If context files are customized to include researcher-defined instructions, rules, or settings, these modifications are similar to prompt development and \must be reported with the same level of detail as other prompts, including their development process and any iterations.
...
Researchers \must summarize in the \paper which tools and skills were exposed to the model, and \should include the complete list (names with one-line purposes), tool schemas, sub-agent definitions, and the identities of any connected Model Context Protocol (MCP) servers as \supplementarymaterial. 
Skills that are defined as a natural-language description of a capability are used by agents in a manner similar to system prompts. Therefore, skills \must be treated as prompts and reported with the same level of detail as other prompts, including their development process and any iterations.
