...
Agentic coding tools expose a range of configuration mechanisms as means for developers to tailor agent behavior including \emph{context files}, skills, subagents, hooks, settings, and rules~\cite{galster2026configuring}.
A \emph{configuration artifact} is a tangible instance of a mechanism: either a single configuration file (e.g., \texttt{CLAUDE.md}, or one subagent file such as \texttt{.claude/agents/reviewer.md}) or a directory bundling several configuration files that together define one artifact (e.g., a skill whose directory contains \texttt{SKILL.md} alongside scripts, references, and assets).
Researchers \must describe which configuration mechanisms were used and \should publish the full contents as \supplementarymaterial, because they steer agent behavior similarly to system prompts.
...
Where subagents are used, researchers \should also describe the delegation pattern: which sub-agent handles which kind of task, and how control returns to the orchestrator.

\paragraph{Tool Catalog and Skill Definitions:}
Agentic systems expose a set of callable tools to the model on each call, along with their schemas: parameter names, types, and natural-language descriptions as the model sees them.
...
The same applies to the list of Model Context Protocol (MCP) servers made available to the model.
Researchers \must summarize in the \paper which tools were exposed to the model, and \should include the complete list (names with one-line purposes), tool schemas, and the identities of any connected Model Context Protocol (MCP) servers as \supplementarymaterial. 
