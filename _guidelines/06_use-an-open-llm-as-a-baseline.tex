...
Besides open models, researchers \removed{\may} can explore whether their study goal can be achieved with existing open-source tools.
...
Finally, unlike \removed{managed cloud} APIs provided by proprietary vendors \added{(e.g., the OpenAPI API)}, installing, configuring, and fine-tuning open-source models can be technically demanding.
...
\added{First, reviewers must distinguish between using LLMs in ways that are central to the research (e.g. building LLM-driven SE tools, using LLMs for synthesis) vs. tangential to the research (e.g. using LLMs as search engines, copy-editors, or to generate recruiting materials). If and only if the LLM use is central to the research, reviewers should expect to see either use of an open LLM as a baseline or a \textit{compelling} argument for why no suitable open LLM exists, or why using an open LLM baseline is impractical for the study. If authors claim to have used an open LLM as a baseline, reviewers should expect some justification as to why the baseline LLM should be considered ``open''. Finally, to the extent that is practical, reviewers should examine the replication package (at minimum, ensure the replication package contains sufficiently detailed instructions; ideally, ensure that following the instructions produces the expected results). Finally, reviewers should \textit{not} criticize studies over open LLM baselines performing significantly differently from proprietary, closed LLMs because this is beyond the authors' control.}
