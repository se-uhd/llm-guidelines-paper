...
Benchmarks, baselines, and metrics play an important role in assessing the effectiveness of LLMs and LLM-based tools. \added{A Benchmark is a ``standard tool for the com-
petitive evaluation and comparison of competing systems
or components according to specific characteristics, such as
performance, dependability, or security''~\cite{} }
Benchmarks are model- and tool-independent standardized tests used to assess the performance of LLMs on specific tasks such as code summarization, code generation, \added{and static analysis}.
...
When selecting benchmarks, it is important to fully understand the benchmark tasks and the expected results because these determine what the benchmark actually assesses.
...
For example, widely used benchmarks such as \emph{HumanEval} \cite{DBLP:journals/corr/abs-2107-03374} and \emph{MBPP} \cite{DBLP:journals/corr/abs-2108-07732} Python functions.
...
For many tasks for which LLMs are being evaluated, there exist traditional non-LLM-based approaches (e.g., for static analysis tasks or program repair) that can serve as a baseline.
...
Due to this non-determinism, researchers \should repeat experiments to statistically assess the performance of a model or tool, e.g., using the arithmetic mean, median, confidence intervals, standard deviations, or more advanced statistical approaches~\cite{DBLP:conf/nips/AgarwalSCCB21}.
...
Two benchmarks used for code generation are \emph{HumanEval} (available on \href{https://github.com/openai/human-eval}{GitHub}) \cite{DBLP:journals/corr/abs-2107-03374} and \emph{MBPP} (available on \href{https://huggingface.co/datasets/google-research-datasets/mbpp}{Hugging Face}) \cite{DBLP:journals/corr/abs-2108-07732}.
...
% TODO Maybe extend this even further
...
% Todo: Extend this and place it more prominent.
...
Researchers \should consider using benchmarks created after the model's training cutoff date or employing dynamic benchmark generation techniques.
...
% Mitigation strategies of benchmark contamination
\added{Mitigation strategies of benchmark contamination can be grouped into three categories: curating new data, refactoring existing data, and benchmark-free evaluation strategies \cite{DBLP:journals/corr/abs-2406-04244}. Creating and curating new uncontaminated benchmark datasets produces unseen benchmark data.
...
\added{Finally, instead of fixed test sets, benchmark-free evaluation uses \judges or \humanvalidation to assess model performance dynamically. While this mitigates contamination since no fixed benchmark is used for evaluation, it introduces subjectivity through evaluator biases and requires additional resources.}
