...
Therefore, it is important to document the entire interaction.
...
\added{As in the previous subsection, in most cases, reviewers should simply request any missing information be added unless so much is missing that the reviewer cannot assess methodological rigor. For this guideline, the trickiest part is the paper's description of how prompts were developed. Generating prompts is a creative, improvised process---characterized by trial-and-error, guesswork, and copious intuitive microdecisions---all of which are impossible to justify with a priori theory or universal principles. The purpose of describing this process is to illuminate potential biases or validity problems; the prompt generating process cannot generally be described algorithmically such that a different researcher following the same process would produce identical prompts. Reviewers should \textit{not} demand justification of each word choice or over-rationalized\footnote{``Over-rationalized'' means presenting chaotic, improvised events as logical plan-execution.} accounts of prompt generation. Instead, reviewers should focus on ensuring that sufficient information is provided such that (1) a new research team could \textit{use} (not produce) exactly the same prompts in exactly the same way; and (2) any potential biases or validity issues are transparent.}
