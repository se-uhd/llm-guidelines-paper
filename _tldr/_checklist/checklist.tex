...
\todo{Update checklist based on final version of the paper}
\todo{Add IDs to recommendations}

The following divides recommendations into essential (MUST) and desirable (SHOULD) properties as a simple summary.

\todo{This is a potential refactoring of the TLDR section into a checklist. Proof of concept.
Still missing is styling of the itemized list (should be checkboxes), and some way to format the headings properly (I ran out of Latex subheadings - we could use chapter instead of section as the top level).
This was originally for the website instead of the paper. For the paper I've added it after the conclusion.
I've also added a possible matrix (with help from my friend Gemini) to show how this might work with the reviewer suggestions. It pulls information from he existing document to populate the check marks, but some omissions seem odd to me (shouldn't they all report model version? ). Also, some "study types" subsections in the guidelines are more about details ("all studies that involve tools with system-level components beyond bare LLMs") which is impossible to map to this.}
...
\todo{figure out why the space is gobbled}
