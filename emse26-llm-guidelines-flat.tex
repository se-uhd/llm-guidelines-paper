...
We distinguish open-weight from open-source LLMs.
An \emph{open-weight} LLM has publicly released trained weights, so others can fine-tune, adapt, or deploy the model~\cite{OSIOpenWeights}.
% GROUNDING OSIOpenWeights: "Open Weights refer to the final weights and biases of a trained neural network. [...] When AI developers share these parameters under an OSI Approved License, they empower others to fine-tune, adapt, or deploy the model for their own projects."
An \emph{open-source} LLM, as defined by the \emph{Open Source Initiative} (OSI)~\cite{OSIAI2024}, also releases the complete source code used to train and run the model and sufficiently detailed information about the training data to build a substantially equivalent model.
...
These components are available under terms that grant the freedoms to use, study, modify, and share the model.
% GROUNDING OSIAI2024: "An Open Source AI is an AI system made available under terms and in a way that grant the freedoms to: Use [...] Study [...] Modify [...] Share"
In our guidelines, an \emph{open LLM} is any LLM that is at least open-weight.
Open-source LLMs are preferable, because the information about their training data lets researchers check for benchmark contamination (see \benchmarksmetrics).
...
For example, \citeauthor{DBLP:journals/corr/abs-2406-09834} evaluated seven advanced LLMs, six of which were open-weight, testing 145 API mappings drawn from eight popular Python libraries across 28,125 completion prompts aimed at detecting deprecated API usage in code completion~\cite{DBLP:journals/corr/abs-2406-09834}.
...
Three evaluated models were open-weight.
...
An open LLM baseline improves reproducibility by exposing model architectures, weights, and ideally training data, enabling independent verification of results.
...
Unlike proprietary alternatives, which can be withdrawn or silently updated, open LLMs remain available for future studies.
...
\begin{itemize}
    \item \emph{Definitional inconsistency.} Many models that claim to be open or open source, such as Meta's \emph{Llama} and Google's \emph{Gemma}, release only trained weights without training data or methodological details~\cite{Gibney2024}, which is why we distinguish open-weight from open-source LLMs (see \emph{Recommendations} above).
    % GROUNDING Gibney2024: "The researchers found that many models that claim to be open or open source [...] including Llama from Meta and Google DeepMind's Gemma [...] are, in fact, just 'open weight'. This means that outside researchers can access and use the trained models, but cannot inspect or customize them. Nor can they fully understand how they were fine-tuned for specific tasks"
    % GROUNDING Gibney2024: "Many of the large language models that power chatbots claim to be open, but restrict access to code and training data."
    ...
    \item \emph{Operational complexity.} Unlike APIs provided by proprietary vendors (e.g., the OpenAI API), installing, configuring, and fine-tuning open models can be technically demanding.
\end{itemize}
...
\begin{itemize}[label={},leftmargin=*]
    ...
    \item \iconS \condition{commercial-models} Include an open-weight LLM, ideally an open-source LLM, as a baseline when using commercial models and report inter-model agreement~(\refopenllm).
    % Open LLM Recommendations: "Empirical studies using LLMs in SE, especially those that target commercial tools or models, \should incorporate an open LLM as a baseline and report established metrics for inter-model agreement"; "In our guidelines, an \emph{open LLM} is any LLM that is at least open-weight"; "Open-source LLMs are preferable". Placed in Model Selection and Configuration because the trigger is a model-choice condition (use of commercial models), even though the recommendation itself involves a baseline.
\end{itemize}
