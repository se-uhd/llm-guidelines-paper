...
\begin{itemize}[label={},leftmargin=*]
    ...
    \item \iconS \condition{commercial-models} Include an open-weight LLM as a baseline when using commercial models and report inter-model agreement~(\refopenllm).
    % Open LLM Recommendations: "Empirical studies using LLMs in SE, especially those that target commercial tools or models, \should incorporate an open LLM as a baseline and report established metrics for inter-model agreement"; "In our guidelines, an \emph{open LLM} is any LLM that is at least open-weight". Placed in Model Selection and Configuration because the trigger is a model-choice condition (use of commercial models), even though the recommendation itself involves a baseline.
\end{itemize}
