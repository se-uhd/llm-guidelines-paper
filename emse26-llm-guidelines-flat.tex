...
\else
  ...
  \newcommand{\benchmarkingtasks}{\href{/study-types/benchmarks/}{\emph{Benchmarking LLMs}}\xspace}
\fi
...
\else
  ...
  \newcommand{\benchmarksmetrics}{\href{/guidelines/benchmarks/}{\emph{Benchmarks and Metrics}}\xspace}
  \newcommand{\openllm}{\href{/guidelines/open-llm/}{\emph{Open LLMs}}\xspace}
  ...
  \newcommand{\refbenchmarks}{\href{/guidelines/benchmarks/}{Benchmarks}}
...
\else
  ...
  \newcommand{\seebenchmarksmetrics}{\href{/guidelines/benchmarks/}{Use Suitable Baselines, Benchmarks, and Metrics}}
...
% \parfillskip can stretch at most 0.75\textwidth, so the last line must
% fill at least ~25% of the text width (raise to 0.8/0.85 for a gentler
% 20%/15% minimum). \emergencystretch absorbs the rare overfull line this
% can cause.
...
Our guidelines name \paper or \supplementarymaterial as the reporting location for each recommendation (see \emph{Reporting Locations} above). The \supplementarymaterial is published according to the \emph{ACM SIGSOFT Open Science Policies}~\cite{daniel_graziotin_2024_10796477} for replication and downstream reuse. In summary, our guidelines apply to SE empirical research broadly, organize recommendations around the taxonomy of seven study types introduced in \studytypes with explicit per-study-type applicability, and pair each recommendation with a target reporting location in \paper or \supplementarymaterial.
% GROUNDING daniel_graziotin_2024_10796477: "the paper itself should, in principle, be made available on the Internet: without any barrier ... under a proper open license that specifies purposes for re-use and repurposing"
...
To ensure the validity and reproducibility of their results, SE studies involving LLMs need empirical guidelines.
...
\citeauthor{DBLP:conf/wsese/BarrosANKKNB25}~\cite{DBLP:conf/wsese/BarrosANKKNB25} conducted a systematic mapping study on using LLMs for qualitative research. The included studies span fields such as healthcare, education, and cultural studies, where LLMs supported qualitative methods, such as grounded theory and thematic analysis, by aiding in pattern identification.
...
% GROUNDING acm2020artifactreview: "Reproducibility (Different team, same experimental setup) ... an independent group can obtain the same result using the author's own artifacts."
...
For agentic systems, \citeauthor{DBLP:conf/kbse/BouzeniaP25}~\cite{DBLP:conf/kbse/BouzeniaP25} unified the runtime trajectories of three SE agents (\emph{RepairAgent}, \emph{AutoCodeRover}, \emph{OpenHands}) into a custom thought-action-result format and released the resulting 120 trajectories with 2,822 LLM interactions as a public dataset.
...
The metric \emph{pass@k} reports the likelihood that a model correctly completes a code snippet at least once within $k$ attempts. Originally introduced by \citeauthor{DBLP:conf/nips/KulalPC0PAL19}~\cite{DBLP:conf/nips/KulalPC0PAL19} as the success rate within a budget of $B$ program attempts (corresponding to $k$ in \emph{pass@k}) and popularized by \citeauthor{DBLP:journals/corr/abs-2107-03374}~\cite{DBLP:journals/corr/abs-2107-03374}, it ranges from 0 (no correct solution in $k$ tries) to 1 (at least one correct solution), with correctness typically defined by passing test cases.
...
\citeauthor{DBLP:journals/corr/abs-2601-11895}'s \emph{DevBench} (available on \href{https://github.com/microsoft/devbench}{GitHub})~\cite{DBLP:journals/corr/abs-2601-11895} synthesizes 1,800 code completion instances from developer telemetry, rather than collecting them from public sources.
...
\citeauthor{DBLP:conf/msr/AhmedDTP25}~\cite{DBLP:conf/msr/AhmedDTP25} proposed a systematic method for deciding whether LLMs can replace human annotators on a given task, using inter-model agreement as an initial screening criterion (with a threshold of $\alpha>0.5$) and model confidence for sample-level decisions. However, their threshold is well below the levels generally considered acceptable for inter-rater agreement.
...
& \rot{\hyperref[sec:llms-as-annotators]{Annotators}} & \rot{\hyperref[sec:llms-as-judges]{Judges}} & \rot{\hyperref[sec:llms-for-synthesis]{Synthesis}} & \rot{\hyperref[sec:llms-as-subjects]{Subjects}} & \rot{\hyperref[sec:studying-llm-usage-in-software-engineering]{Usage}} & \rot{\hyperref[sec:llms-for-new-software-engineering-tools]{Tools}} & \rot{\hyperref[sec:benchmarking-llms-for-software-engineering-tasks]{Benchmarks}} \\
\midrule
...
\begin{itemize}[label={},leftmargin=*]
    ...
    % G4 Interaction Logs: "For agentic systems, interaction logs cover the human-facing exchanges with the agent, including human-in-the-loop feedback, approval or rejection decisions, and iterative refinements. These \should also be reported as \supplementarymaterial".
    ...
    % G4 Agentic Plans: "researchers \should report any plans the system exposes as \supplementarymaterial".
\end{itemize}
...
\begin{itemize}[label={},leftmargin=*]
    ...
    % G7 Study Design Considerations: "When designing custom instruments to assess LLM output (e.g., questionnaires, scales), researchers \must share these instruments".
    ...
    \item \iconS \condition{human-validation} When LLMs replace humans in research tasks, report in the \paper the systematic approach used to justify the replacement, including inter-model and model-to-human agreement~(\refhumanvalidation).
    % G7 Recommendations (Replacing Human Judgment): "researchers \should follow systematic approaches to support this judgment, e.g., showing that a jury of three LLMs exhibits inter-model agreement... such as Krippendorff's $\alpha>0.8$"; "researchers \should additionally validate a sample against human experts"; "Researchers \should justify why the LLM is methodologically appropriate".
...
\begin{itemize}[label={},leftmargin=*]
    ...
    % G8 Ethical \& Regulatory: "Studies involving sensitive data \must discuss data governance mechanisms tailored toward LLM environments, compliant with applicable jurisdictional obligations".
    \item \iconS Justify LLM usage in light of its resource demands~(\reflimitations).
    % G8 Environmental \& Sustainability: "Researchers \should justify the LLM's resource consumption against the benefits over traditional approaches".
    \item \iconS Provide a full replication package as \supplementarymaterial, including step-by-step instructions for verifying and reproducing the results~(\reflimitations).
    % G8 Reliability \& Reproducibility Mitigations: "Mitigations include \emph{replication packages} covering prompt and architecture specifications, model outputs, and representative examples for partial replicability (ideally accompanied by an implementation using an open model for long-term stability)". G6's narrower text now reads "Researchers \should ensure the open-LLM baseline is independently reproducible from their \supplementarymaterial".
...
\begin{itemize}[label={},leftmargin=*]
    ...
    % G8 External Validity: "Researchers \must discuss the limitations and mitigations of external validity"; covers cross-model transfer and cross-time instability.
