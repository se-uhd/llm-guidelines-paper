...
% This rating can produce total or partial ordersBeyond acting as absolute raters, LLMs naturally function as rankers. For example, 
They can compare two code solutions to pick the best readable solution, or sort multiple candidates by quality.
Judgment is distinct from the 
% more qualitative 
task of assigning a code or label to unstructured text (see Section \annotators).
...
LLMs can be much more efficient and scale more easily than the equivalent human approach. \added{These advantages are similar to those described in Section \annotators.  }
For large-scale studies, human evaluators often experience fatigue and reduced concentration, which can undermine the quality and consistency of their evaluations. 
In contrast, LLMs can maintain consistent performance over extended periods regardless of the workload.
\added{As a result LLM automation supports larger datasets for judgment studies}.
...
The underlying statistical framework of an LLM usually pushes outputs towards the most likely (majority) answer, which might be incorrect.
