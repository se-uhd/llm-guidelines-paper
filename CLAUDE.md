...
Academic paper accepted by the Empirical Software Engineering (EMSE) journal. The paper presents guidelines for empirical studies in software engineering involving LLMs. It is co-authored by 22 researchers and synced with Overleaf via git.
...
**Run-in guideline captions:** `\guidelineparagraph{Title}` renders the run-in paragraph captions inside guideline bodies. The paper branch appends the trailing period (run-in caption and body share a line); the website branch emits the bare title so converted headings carry no trailing punctuation. Type no punctuation in the argument. The mention rule above (`\paragraph`, `\studytypeparagraph`) covers these captions too.
